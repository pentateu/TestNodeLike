...
char immediate_check_handle_key = 0;
char immediate_idle_handle_key = 0;
...
-(void) stop{
    ...
    //stop event loop
    uv_loop_t * eventLoop = [NLContext eventLoopInContext:self];
    uv_stop(eventLoop);
    
    //clean-up stuff
    uv_check_t *immediate_check_handle = ((NSValue *)[self.virtualMachine nodelikeGet:&immediate_check_handle_key]).pointerValue;
    free(immediate_check_handle);
    
    uv_idle_t  *immediate_idle_handle  = ((NSValue *)[self.virtualMachine nodelikeGet:&immediate_idle_handle_key]).pointerValue;
    free(immediate_idle_handle);
    
    //clean up virtual machine
    [self.virtualMachine nodelikeSet:&env_event_loop toValue:nil];
    [self.virtualMachine nodelikeSet:&immediate_check_handle_key toValue:nil];
    [self.virtualMachine nodelikeSet:&immediate_idle_handle_key toValue:nil];
    [self.virtualMachine nodelikeSet:&env_process_object toValue:nil];
    
    //free event loop stuff
    uv_loop_delete(eventLoop);
}
...
+ (void)attachToContext:(JSContext *)context {
    ...
    //store these objs.. so they can be freed-up at the end
    [context.virtualMachine nodelikeSet:&immediate_check_handle_key toValue:[NSValue valueWithPointer:immediate_check_handle]];
    [context.virtualMachine nodelikeSet:&immediate_idle_handle_key toValue:[NSValue valueWithPointer:immediate_idle_handle]];
